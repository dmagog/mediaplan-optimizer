...
работает вместе: три демонстрации из кейса проходят, 50 тестов зелёные,
поднимается одной командой `docker compose up`. Код лежит здесь:
https://github.com/avanturer/mediaplan-optimizer
...
120–180 тысяч. Если у команды двадцать-сорок таких кампаний в год, это уже
от полумиллиона до двух, не считая времени людей.
...
событий мало, поэтому окна длиннее и к порогу добавлен тест значимости на
3σ. Честное следствие: падение CR на 40 % в маленьком канале за оставшиеся
...
Симулятор: три семейства каналов, восемь сценариев шоков, шок из интерфейса
...
цена следующей конверсии выровнена около 1 070 ₽ во всех каналах, которые не
упёрлись в потолок. Считается за 0,1 с.

Демо 2: 50 000 кликов за 14 дней. На всех каналах достижимо за 288 тысяч. На
узком пресете из четырёх каналов отказ: максимум 37 927 кликов, связывает
...
Детектор: без шока 0,14 срабатывания на кампанию, часть из них
...
Аудитории каналов у нас не пересекаются (в презентации МТС AdTech пересечение
трёх каналов по GFD 5,3 %). Цена задаётся ландшафтом, а не аукционом второй
цены. Охват по случайному размещению, а не NBD. Конверсии мгновенные. VTR по
допущению. MAPE по KPI как среднее по часам около 21 % у всех стратегий из-за
разброса миров, хотя в конце
кампании 12 %. И главное: каналы абстрактные, наши CPM и CTR не являются
...
Лимит полномочий 100 000 ₽ в демо стоит только для показа сценария.
