...
и промахом в каждой четвёртой набегает от 0,5 до 2 млн ₽ в год и десятки
человеко-дней. Расчёт — в [описании проекта](docs/PROJECT.md).
...
четырёх стратегий на 20 парных мирах и 5 сценариях — около восьми минут, на
ста мирах около сорока (время каждого прогона пишется в конец
`results/report.md`).
...
| Снимки кабинета и гифка плеера: пятнадцать экранов обоих кабинетов | [docs/figures/cabinet](docs/figures/cabinet) | `python scripts/shoot_cabinet.py` (нужны Playwright и Pillow) |
...
плану целиком, недостижимая цель типа B с готовыми ходами, кампания по часам с
...
| Медиаплан | распределение по каналам с прогнозом и разбросом; недостижимую цель отклоняет и даёт варианты с посчитанным бюджетом; если бюджет размещён не весь, говорит почему |
...
| **С6** | Кампания закончилась, нужен разговор с клиентом | Собирает итог: обещали, получили, что дало ведение против плана без изменений, и сравнивает стратегии на двадцати парных мирах прямо в кабинете (сотня миров — офлайн-стенд) |
...
Числа в таблицах выше переносит из артефактов `python scripts/sync_doc_numbers.py`
(с `--check` только сверяет). Таблицу режимов считает
`python scripts/run_modes.py --seeds 100`, гистограммы
перерисовывает `python scripts/build_stand_figures.py` из уже посчитанного
`results/comparison.json` (нужен matplotlib, в зависимости проекта он не входит).
Какой режим
включать — решение клиента, разбор в
...
`results/ml_validation.md`. Включается она в стенде и скриптах, не в кабинете:
поля `ml` у HTTP-API нет.
...
[docs/world/EXTENSIONS.md](docs/world/EXTENSIONS.md). Почему
